...
export default function Devices() {
  ...
  const buildings = useMemo(() => {
    const set = new Set(mockDevices.map(d => d.building));
    return Array.from(set);
  }, []);

  const filteredDevices = useMemo(() => {
    return mockDevices.filter(device => {
      if (searchText && !device.deviceNo.toLowerCase().includes(searchText.toLowerCase())) {
        return false;
      }
      if (buildingFilter !== 'all' && device.building !== buildingFilter) {
        return false;
      }
  ...
  }, [searchText, buildingFilter, statusFilter]);
  ...
  const deviceMapPoints = useMemo(() => {
    return filteredDevices.map((device, index) => {
      const cols = 6;
      const rows = Math.ceil(filteredDevices.length / cols);
  ...
  const handleToggleDevice = (device: Device) => {
    console.log('Toggle device:', device.deviceNo, device.status === 'online' ? 'off' : 'on');
  };
  ...
  const handleSavePrice = (deviceId: string) => {
    console.log('Save price for device:', deviceId, 'price:', tempPrice);
    setPriceEditDevice(null);
  };
                ...
                {pagedDevices.map((device) => (
                  <tr key={device.id} className="hover:bg-neutral-50 transition-colors">
                    <td className="table-td">
                      <span className="font-medium text-neutral-900">{device.deviceNo}</span>
                    </td>
                    <td className="table-td">
                    ...
                    <td className="table-td">
                      <div className="flex items-center gap-2 min-w-[120px]">
                        <div className={cn('flex-1 h-2 rounded-full overflow-hidden', getWaterLevelBg(device.waterLevel))}>
                          <div
                            className={cn('h-full rounded-full transition-all duration-500', getWaterLevelColor(device.waterLevel))}
                            style={{ width: `${device.waterLevel}%` }}
                          />
                        </div>
                        <span className={cn('text-xs font-medium w-10 text-right', getWaterLevelText(device.waterLevel))}>
                          {device.waterLevel}%
                        </span>
                      </div>
                    </td>
                    <td className="table-td">
                      <span className="text-neutral-700">{device.tds} ppm</span>
                    </td>
                    <td className="table-td">
                      <span className="text-neutral-700">{device.ph}</span>
                    </td>
                    <td className="table-td">
                      <span className="text-neutral-700">{device.temperature}°C</span>
                    </td>
                    <td className="table-td">
                      {priceEditDevice === device.id ? (
                        <div className="flex items-center gap-1">
                          <input
                            ...
                            className="w-16 px-2 py-1 border border-neutral-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-primary-400 focus:border-transparent"
                          />
                    ...
                    <td className="table-td">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          ...
                          className="p-2 rounded-md text-neutral-500 hover:text-primary-600 hover:bg-primary-50 transition-all"
                          title="查看详情"
                        >
                        ...
                        <button
                          ...
                          title={device.status === 'online' ? '远程停止' : '远程启动'}
                        >
                        ...
                        {priceEditDevice === device.id ? (
                          <div className="flex items-center gap-0.5">
                            <button
                              onClick={() => handleSavePrice(device.id)}
                              className="p-2 rounded-md text-success-600 hover:bg-success-50 transition-all"
                              title="保存"
                            >
                            ...
                            <button
                              ...
                              className="p-2 rounded-md text-neutral-400 hover:bg-neutral-100 transition-all"
                              title="取消"
                            >
                              <X className="w-4 h-4" />
                            </button>
                          ...
                          <button
                            onClick={() => handleStartPriceEdit(device)}
                            className="p-2 rounded-md text-neutral-500 hover:text-accent-600 hover:bg-accent-50 transition-all"
                            title="配置价格"
                          >
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-4 py-3 border-t border-neutral-100">
              <span className="text-sm text-neutral-500">
                共 {filteredDevices.length} 条，第 {currentPage}/{totalPages} 页
              </span>
              <div className="flex items-center gap-1">
                <button
                  onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
                  disabled={currentPage === 1}
                  className="p-2 rounded-md border border-neutral-200 text-neutral-500 hover:bg-neutral-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                >
            ...
            {deviceMapPoints.map((device) => (
              ...
              >
                <div
                  onClick={() => handleOpenDetail(device)}
                  className={cn(
                    'relative w-5 h-5 rounded-full ring-2 ring-white cursor-pointer transition-transform hover:scale-150',
                    deviceStatusColorMap[device.status]
                  )}
                >
                  <div
                    className={cn(
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="bg-neutral-50 rounded-xl p-4 text-center">
                    ...
                    <p className="text-lg font-bold text-neutral-900">
                      {Math.floor(Math.random() * 2000 + 500)}h
                    </p>
                  </div>
                  <div className="bg-neutral-50 rounded-xl p-4 text-center">
                    ...
                    <p className="text-lg font-bold text-neutral-900">
                      {formatWaterAmount(selectedDevice.totalWater)}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="bg-neutral-50 rounded-xl p-5 flex flex-col items-center">
                  ...
                  <p className="mt-4 text-sm text-neutral-500 text-center">
                    水箱容量 200L，剩余约 {Math.round(200 * selectedDevice.waterLevel / 100)}L
                  </p>
